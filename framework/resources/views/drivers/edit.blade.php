...
<div class="row">
  <div class="col-md-12">
    ...
    {!! Form::open([
    'route' => ['drivers.send', $driver->id],  // <--- FIXED HERE
    'files' => true,
    'method' => 'PATCH'
]) !!}
    {!! Form::hidden('id', $driver->id) !!} {{-- <--- FIXED HERE --}}
    {!! Form::hidden('edit', $driver->id) !!}
    {!! Form::hidden('detail_id', $driver->getMeta('id')) !!}
    {!! Form::hidden('user_id', $driver->id) !!} {{-- This can remain as the logged-in user --}}
        ...
        <div class="row">
            ...
            {{-- Driver Commission Value (Shown conditionally via JS) --}}
            ...
            <div class="col-md-4">
              <div class="form-group">
                {!! Form::label('owner_id', __('fleet.selectVowner'), ['class' => 'form-label']) !!}
                <select id="owner_id" name="owner_id" class="form-control w-100" required>
                  <option value="">@lang('fleet.selectVowner')</option>
                  @foreach ($driver_vehicle_rows as $row)
                      @php
                          $selectedValue = $selected_owner_id . '|' . $selected_license_plate;
                          $optionValue = $row->vowner_id . '|' . $row->license_plate;
                      @endphp
                      <option value="{{ $optionValue }}" {{ $optionValue == $selectedValue ? 'selected' : '' }}>
                          {{ $row->name }} - {{ $row->license_plate }}
                      </option>
                  @endforeach
                </select>
              </div>
