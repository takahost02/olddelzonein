              ...
              <tr>
                ...
                <td>
                  @if(!in_array($row,["Inquiries","Reports","Settings"]))
                    @php
                      try {
                        $checked = $data->hasPermissionTo($row." list") ? 'checked' : '';
                      } catch (\Spatie\Permission\Exceptions\PermissionDoesNotExist $e) {
                        $checked = '';
                      }
                    @endphp
                    <input type="checkbox" name="{{ $row." list" }}" value="1" class="flat-red form-control {{ $row }}" {{ $checked }}>
                ...
                <td>
                  @if(!in_array($row,["Inquiries","Reports","Transactions","ServiceReminders","Settings"]))
                    @php
                      try {
                        $checked = $data->hasPermissionTo($row." edit") ? 'checked' : '';
                      } catch (\Spatie\Permission\Exceptions\PermissionDoesNotExist $e) {
                        $checked = '';
                      }
                    @endphp
                    
                    <input type="checkbox" name="{{ $row." edit" }}" value="1" class="flat-red form-control {{ $row }}" {{ $checked }}>
                ...
                <td>
                  @if(!in_array($row,["Inquiries","Reports","Settings"]))
                    @php
                      try {
                        $checked = $data->hasPermissionTo($row." delete") ? 'checked' : '';
                      } catch (\Spatie\Permission\Exceptions\PermissionDoesNotExist $e) {
                        $checked = '';
                      }
                    @endphp
                    
                    <input type="checkbox" name="{{ $row." delete" }}" value="1" class="flat-red form-control {{ $row }}" {{ $checked }}>
