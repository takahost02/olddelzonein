              ...
              <div class="col-md-4">
               ...
               <div class="form-group">
                    ...
                    <div class="col-xs-6">
                        ...
                        <small id="license-error" class="text-danger" style="display: none;"></small>
                    </div>
                </div>
               ...
               <div class="form-group">
                  ...
                  <select id="vowner_id" name="vowner_id" class="form-control w-100" required>
                    
                    @foreach($vowners as $vowner)
                      <option value="{{ $vowner->id }}">
    ...
    $('#license_plate').on('input', function () {
        ...
        if (plate.length > 0) {
            $.ajax({
                ...
                success: function (data) {
                    if (!data.unique) {
                        // Show error message and mark invalid
                        $('#license-error').text('License plate already exists!').show();
                        $('#license_plate').addClass('is-invalid');
                        // Disable submit button
                        $('button[type="submit"]').attr('disabled', true);
                    } else {
                        // Hide error and mark valid
                        $('#license-error').hide();
                ...
                error: function (xhr, status, error) {
                    ...
                    $('#license-error').text('Error checking license plate. Please try again.').show();
                    $('#license_plate').addClass('is-invalid');
                    $('button[type="submit"]').attr('disabled', true);
                }
            });
