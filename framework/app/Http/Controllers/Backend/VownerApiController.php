...
class VownerApiController extends Controller {
	public function import_records(Request $request) {
		...
		if (count($errors) > 0) {
			...
			try {
				...
				Excel::import(new VownerImport, 'assets/samples/' . $fileName);
				// $excel = Importer::make('Excel');
				// $excel->load('assets/samples/' . $fileName);
				// $collection = $excel->getCollection()->toArray();
				// array_shift($collection);
				// // dd($collection);
				// foreach ($collection as $vowner) {
				//     if ($vowner[4] != null) {
				//         $id = User::create([
				//             "name" => $vowner[0] . " " . $vowner[2],
				//             "email" => $vowner[4],
				//             "password" => bcrypt($vowner[15]),
				//             "user_type" => "D",
